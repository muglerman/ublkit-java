...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:key name="by-invoiceLine-id"
           match="*[local-name()='SummaryDocuments']/sac:SummaryDocumentsLine"
           use="cbc:LineID"/>
  <!-- key tributos duplicados por linea -->
  <xsl:key name="by-tributos-in-line"
           match="sac:SummaryDocumentsLine/cac:TaxTotal/cac:TaxSubtotal"
           use="concat(cac:TaxCategory/cac:TaxScheme/cbc:ID,'-', ../../cbc:LineID)"/>
  <!-- key BillingPayment duplicados por linea -->
  <xsl:key name="by-BillingPayment-in-line"
           match="sac:SummaryDocumentsLine/sac:BillingPayment"
           use="concat(cbc:InstructionID,'-', ../cbc:LineID)"/>
  <!-- key ChargeIndicator duplicados por linea -->
  <xsl:key name="by-ChargeIndicator-in-line"
           match="sac:SummaryDocumentsLine/cac:AllowanceCharge"
           use="concat(cbc:ChargeIndicator,'-', ../cbc:LineID)"/>
  <!-- key comprobantes duplicados por linea -->
  <xsl:param name="nombreArchivoEnviado"/>
  <xsl:key name="by-bills-in-line"
           match="sac:SummaryDocumentsLine"
           use="concat(cbc:DocumentTypeCode, '-', substring(cbc:ID,1,4), '-', number(substring(cbc:ID,6)), '-', cac:Status/cbc:ConditionCode )"/>
  <xsl:template match="/*">
    <!-- 
        ===========================================================================================================================================
        Variables  
    ...
    <xsl:variable name="idFilename"
                  select="substring($nombreArchivoEnviado, 13, string-length($nombreArchivoEnviado) - 16)"/>
    <xsl:variable name="fechaEnvioFile"
                  select="substring($nombreArchivoEnviado, 16, 8)"/>
    <!-- 
        ===========================================================================================================================================
        Variables  
        ===========================================================================================================================================
    ...
    <xsl:call-template name="isTrueExpresion">
      ...
      <xsl:with-param name="node"
                      select="cbc:ID"/>
      <xsl:with-param name="expresion"
                      select="$fechaEnvioFile != translate(cbc:IssueDate,'-','')"/>
    </xsl:call-template>
    <!-- /SummaryDocuments/cbc:ReferenceDate El valor del Tag UBL es mayor a la "Fecha de generación del resumen"
        ERROR 4036 -->
	<!-- 
    <xsl:call-template name="isDateBefore">
      <xsl:with-param name="errorCodeValidate"
                      select="'4036'"/>
      <xsl:with-param name="startDateNode"
                      select="cbc:IssueDate"/>
      <xsl:with-param name="endDateNode"
                      select="cbc:ReferenceDate"/>
    </xsl:call-template>
	 -->
    ...
    <xsl:call-template name="isTrueExpresion">
      <xsl:with-param name="errorCodeValidate"
                      select="'1034'"/>
      <xsl:with-param name="node"
                      select="cac:AccountingSupplierParty/cbc:CustomerAssignedAccountID"/>
      <xsl:with-param name="expresion"
                      select="$numeroRuc != cac:AccountingSupplierParty/cbc:CustomerAssignedAccountID"/>
    </xsl:call-template>
    <!-- 
        ===========================================================================================================================================
    ...
    <xsl:call-template name="isTrueExpresion">
      <xsl:with-param name="errorCodeValidate"
                      select="'0158'"/>
      <xsl:with-param name="node"
                      select="sac:SummaryDocumentsLine[last()]/cbc:LineID"/>
      <xsl:with-param name="expresion"
                      select="count(sac:SummaryDocumentsLine) &gt; 500"/>
      <xsl:with-param name="descripcion"
                      select="concat('No se deben de enviar mas de 500 comprobantes para el resumen: ', cbc:ID)"/>
    </xsl:call-template>
  ...
  <xsl:template match="cac:AccountingSupplierParty">
    ...
    <xsl:call-template name="existElement">
      <xsl:with-param name="errorCodeNotExist"
                      select="'2217'"/>
      <xsl:with-param name="node"
                      select="cbc:CustomerAssignedAccountID"/>
    </xsl:call-template>
    ...
    <xsl:call-template name="existAndRegexpValidateElement">
      ...
      <xsl:with-param name="errorCodeValidate"
                      select="'2218'"/>
      <xsl:with-param name="node"
                      select="cbc:AdditionalAccountID"/>
      <xsl:with-param name="regexp"
                      select="'^(6)$'"/>
  ...
  <xsl:template match="sac:SummaryDocumentsLine">
    ...
    <xsl:call-template name="isTrueExpresion">
      ...
      <xsl:with-param name="node"
                      select="cbc:LineID"/>
      <xsl:with-param name="expresion"
                      select="count(key('by-invoiceLine-id', number(cbc:LineID))) &gt; 1"/>
      <xsl:with-param name="descripcion"
                      select="concat('Error en la linea: ', position(), '. ')"/>
    </xsl:call-template>
    ...
    <xsl:call-template name="existElement">
      <xsl:with-param name="errorCodeNotExist"
                      select="'2522'"/>
      <xsl:with-param name="node"
                      select="cac:Status/cbc:ConditionCode"/>
    </xsl:call-template>
    ...
    <xsl:call-template name="isTrueExpresion">
      ...
      <xsl:with-param name="node"
                      select="cbc:ID"/>
      <xsl:with-param name="expresion"
                      select="count(key('by-bills-in-line', concat(cbc:DocumentTypeCode, '-', substring(cbc:ID,1,4), '-', number(substring(cbc:ID,6)), '-', cac:Status/cbc:ConditionCode ))) &gt; 1"/>
      <xsl:with-param name="descripcion"
    ...
    <xsl:if test="cac:Status/cbc:ConditionCode ='1'">
      <xsl:call-template name="isTrueExpresion">
        ...
        <xsl:with-param name="node"
                        select="cbc:ID"/>
        <xsl:with-param name="expresion"
                        select="count(key('by-bills-in-line', concat(cbc:DocumentTypeCode, '-', substring(cbc:ID,1,4), '-', number(substring(cbc:ID,6)), '-', '2' ))) &gt; 0"/>
        <xsl:with-param name="descripcion"
                        select="concat('Error en la linea: ', $nroLinea, '. ')"/>
      </xsl:call-template>
    </xsl:if>
    ...
    <xsl:if test="cac:Status/cbc:ConditionCode ='2'">
      <xsl:call-template name="isTrueExpresion">
        ...
        <xsl:with-param name="node"
                        select="cbc:ID"/>
        <xsl:with-param name="expresion"
                        select="count(key('by-bills-in-line', concat(cbc:DocumentTypeCode, '-', substring(cbc:ID,1,4), '-', number(substring(cbc:ID,6)), '-', '3' ))) &gt; 0"/>
        <xsl:with-param name="descripcion"
                        select="concat('Error en la linea: ', $nroLinea, '. ')"/>
      </xsl:call-template>
    </xsl:if>
    ...
    <xsl:call-template name="existAndRegexpValidateElement">
      ...
      <xsl:with-param name="node"
                      select="cbc:ID"/>
      <xsl:with-param name="regexp"
                      select="'^([B][A-Z0-9]{3}|[\d]{1,4})-(?!0+$)([0-9]{1,8})$'"/>
    ...
    <xsl:call-template name="existAndValidateValueTwoDecimal">
      ...
      <xsl:with-param name="errorCodeValidate"
                      select="'2251'"/>
      <xsl:with-param name="node"
                      select="sac:TotalAmount"/>
      <xsl:with-param name="isGreaterCero"
                      select="false()"/>
      <xsl:with-param name="descripcion"
                      select="concat('Error en la linea: ', $nroLinea, '. ')"/>
    </xsl:call-template>
    ...
    <xsl:variable name="totVentaOservicioPrestado"
                  select="sac:TotalAmount"/>
    <!-- 
    ...
    <xsl:if test="$totVentaOservicioPrestado">
      <xsl:variable name="sumaCargosTributos"
                    select="number($totOtrosTributos) + number($totIGV) + number($totISC) + number($totOtrosCargos)  + number($totICBPER) + number($totVentaOperInafectas) + number($totVentaOperExoneradas) + number($totVentaOperGravadas)"/>
      <xsl:call-template name="isTrueExpresion">
        <xsl:with-param name="errorCodeValidate"
                        select="'4027'"/>
        <xsl:with-param name="node"
                        select="sac:TotalAmount"/>
        <xsl:with-param name="expresion"
                        select="($totVentaOservicioPrestado + 5 ) &lt; $sumaCargosTributos or ($totVentaOservicioPrestado - 5) &gt; $sumaCargosTributos"/>
        <xsl:with-param name="isError"
                        select="false()"/>
        <xsl:with-param name="descripcion" select="concat('Error en la linea: ', $totVentaOservicioPrestado ,'-',$sumaCargosTributos , '. ')"/>
      </xsl:call-template>
    </xsl:if>
    ...
    <xsl:variable name="monedaComprobante"
                  select="sac:TotalAmount/@currencyID"/>
    <xsl:variable name="nodosConMonedaDiferente"
                  select="descendant::*[@currencyID != $monedaComprobante and not(ancestor-or-self::sac:SUNATPerceptionSummaryDocumentReference)]"/>
    <xsl:call-template name="isTrueExpresion">
      <xsl:with-param name="errorCodeValidate"
                      select="'2071'"/>
      <xsl:with-param name="node"
                      select="$nodosConMonedaDiferente"/>
      <xsl:with-param name="expresion"
                      select="count($nodosConMonedaDiferente)&gt;0"/>
      <xsl:with-param name="descripcion"
                      select="concat('Error en la linea: ', $nroLinea, '. ')"/>
    </xsl:call-template>
    ...
    <xsl:for-each select="sac:BillingPayment">
      ...
      <xsl:call-template name="isTrueExpresion">
        ...
        <xsl:with-param name="node"
                        select="cbc:InstructionID"/>
        <xsl:with-param name="expresion"
                        select="count(key('by-BillingPayment-in-line', concat(cbc:InstructionID,'-', $nroLinea))) &gt; 1"/>
        <xsl:with-param name="descripcion"
                        select="concat('Error en la linea: ', $nroLinea, '. ')"/>
      </xsl:call-template>
    </xsl:for-each>
    <xsl:for-each select="cac:AllowanceCharge">
      ...
      <xsl:call-template name="isTrueExpresion">
        ...
        <xsl:with-param name="node"
                        select="cbc:ChargeIndicator"/>
        <xsl:with-param name="expresion"
                        select="count(key('by-ChargeIndicator-in-line', concat(cbc:ChargeIndicator,'-', $nroLinea))) &gt; 1"/>
        <xsl:with-param name="descripcion"
                        select="concat('Error en la linea: ', $nroLinea, '. ')"/>
      </xsl:call-template>
    ...
    <xsl:call-template name="isTrueExpresion">
      ...
      <xsl:with-param name="expresion"
                      select="count(cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme/cbc:ID[text()='1000']) = 0 "/>
      <xsl:with-param name="descripcion"
                      select="concat('Error en la linea: ', $nroLinea, '. ')"/>
    </xsl:call-template>
    ...
    <xsl:if test="sac:SUNATPerceptionSummaryDocumentReference and (cbc:DocumentTypeCode!='03' or cac:Status/cbc:ConditionCode='2')">
      <xsl:call-template name="rejectCall">
        ...
        <xsl:with-param name="errorMessage"
                        select="concat('Error en la linea: ', $nroLinea, '. Solo se acepta informacion de percepcion para nuevas boletas: el codigo de operacion es: ', cac:Status/cbc:ConditionCode,' y debe de ser 1.')"/>
      </xsl:call-template>
    </xsl:if>
    <xsl:if test="sac:SUNATPerceptionSummaryDocumentReference and cbc:DocumentTypeCode='03'">
      ...
      <xsl:call-template name="existElement">
        <xsl:with-param name="errorCodeNotExist"
                        select="'2679'"/>
        <xsl:with-param name="node"
                        select="cac:AccountingCustomerParty/cbc:CustomerAssignedAccountID"/>
        <xsl:with-param name="descripcion"
                        select="concat('Error en la linea: ', $nroLinea, '. ')"/>
      </xsl:call-template>
    ...
    <xsl:if test="(not(cac:BillingReference) and (cbc:DocumentTypeCode='07' or cbc:DocumentTypeCode='08')) or (cac:BillingReference and not(cbc:DocumentTypeCode='07' or cbc:DocumentTypeCode='08'))">
      <xsl:call-template name="rejectCall">
        <xsl:with-param name="errorCode"
                        select="'2512'"/>
        <xsl:with-param name="errorMessage"
                        select="concat('Error en la linea: ', $nroLinea, '. Solo se acepta informacion de comprobantes de referencia para notas (Credito o debito): el tipo de comprobante es: ', cbc:DocumentTypeCode,' y debe de ser 07 o 08.')"/>
      </xsl:call-template>
    </xsl:if>
  ...
  <xsl:template match="cac:TaxTotal"
    ...
    <xsl:variable name="tipoOperacion"
                  select="$root/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/sac:AdditionalInformation/sac:SUNATTransaction/cbc:ID"/>
    ...
    <xsl:call-template name="isTrueExpresion">
      ...
      <xsl:with-param name="node"
                      select="cbc:TaxAmount"/>
      <xsl:with-param name="expresion"
                      select="number(cac:TaxSubtotal/cbc:TaxAmount) != number(cbc:TaxAmount)"/>
      <xsl:with-param name="descripcion"
                      select="concat('Error en la linea: ', $nroLinea, '. ')"/>
    </xsl:call-template>
    ...
    <xsl:call-template name="existElement">
      <xsl:with-param name="errorCodeNotExist"
                      select="'2271'"/>
      <xsl:with-param name="node"
                      select="cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme/cbc:Name"/>
      <xsl:with-param name="descripcion"
                      select="concat('Error en la linea: ', $nroLinea, '. ')"/>
    </xsl:call-template>
    <xsl:choose>
      <xsl:when test="cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme/cbc:ID = '1000'">
        ...
        <xsl:call-template name="isTrueExpresion">
          ...
          <xsl:with-param name="node"
                          select="cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme/cbc:ID"/>
          <xsl:with-param name="expresion"
                          select="cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme[cbc:ID = '1000']/cbc:Name != 'IGV'"/>
          <xsl:with-param name="descripcion"
                          select="concat('Error en la linea: ', $nroLinea, '. cbc:Name debe de ser IGV')"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:when test="cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme/cbc:ID = '2000'">
        ...
        <xsl:call-template name="isTrueExpresion">
          ...
          <xsl:with-param name="node"
                          select="cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme/cbc:ID"/>
          <xsl:with-param name="expresion"
                          select="cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme[cbc:ID = '2000']/cbc:Name != 'ISC'"/>
          <xsl:with-param name="descripcion"
                          select="concat('Error en la linea: ', $nroLinea, '. ')"/>
        </xsl:call-template>
      </xsl:when>
  ...
  <xsl:template match="sac:SUNATPerceptionSummaryDocumentReference">
    ...
    <xsl:call-template name="findElementInCatalog">
      <xsl:with-param name="catalogo"
                      select="'22'"/>
      <xsl:with-param name="idCatalogo"
                      select="sac:SUNATPerceptionSystemCode"/>
      <xsl:with-param name="errorCodeValidate"
                      select="'2517'"/>
    </xsl:call-template>
    ...
    <xsl:call-template name="findElementInCatalogProperty">
      ...
      <xsl:with-param name="valorPropiedad"
                      select="number(sac:SUNATPerceptionPercent)"/>
      <xsl:with-param name="errorCodeValidate"
                      select="'2891'"/>
    </xsl:call-template>
	 -->
    ...
    <xsl:call-template name="existAndValidateValueTwoDecimal">
      ...
      <xsl:with-param name="errorCodeValidate"
                      select="'2893'"/>
      <xsl:with-param name="node"
                      select="cbc:TotalInvoiceAmount"/>
      <xsl:with-param name="isGreaterCero"
                      select="true()"/>
      <xsl:with-param name="descripcion"
                      select="concat('Error en la linea: ', $parent_position,'. Monto total de la percepción debe de ser un numero valido, como maximo dos decimales; mayor que cero')"/>
    </xsl:call-template>
    ...
    <xsl:variable name="sumaTotalCobrarMasPercepcion"
                  select="number(cbc:TaxableAmount) + number(cbc:TotalInvoiceAmount)"/>
    ...
    <xsl:if test="number(number(sac:SUNATTotalCashed) + 1) &lt; number($sumaTotalCobrarMasPercepcion) or number(number(sac:SUNATTotalCashed) - 1) &gt; number($sumaTotalCobrarMasPercepcion)">
      <xsl:call-template name="addWarning">
        ...
        <xsl:with-param name="warningMessage"
                        select="concat('Error en la linea: ', $parent_position,'. Monto total a cobrar incluida la percepción no coincide: ',sac:SUNATTotalCashed, ' es diferente a la suma del Monto total de la percepción y la Base imponible percepción: ', $sumaTotalCobrarMasPercepcion)"/>
      </xsl:call-template>
    </xsl:if>
    ...
    <xsl:call-template name="existAndRegexpValidateElement">
      ...
      <xsl:with-param name="errorCodeValidate"
                      select="'2685'"/>
      <xsl:with-param name="node"
                      select="cbc:TotalInvoiceAmount/@currencyID"/>
      <xsl:with-param name="regexp"
                      select="'^(PEN)$'"/>
    </xsl:call-template>
    ...
    <xsl:call-template name="existAndRegexpValidateElement">
      ...
      <xsl:with-param name="errorCodeValidate"
                      select="'2690'"/>
      <xsl:with-param name="node"
                      select="sac:SUNATTotalCashed/@currencyID"/>
      <xsl:with-param name="regexp"
                      select="'^(PEN)$'"/>
    </xsl:call-template>
  </xsl:template>
  ...
  <xsl:template match="cac:BillingReference">
    <xsl:param name="nroLinea"/>
    <xsl:variable name="tipoComprobanteReferencia"
                  select="cac:InvoiceDocumentReference/cbc:DocumentTypeCode"/>
    <xsl:variable name="comprobanteModificaID"
                  select="cac:InvoiceDocumentReference/cbc:ID"/>
    ...
    <xsl:call-template name="existAndRegexpValidateElement">
      ...
      <xsl:with-param name="errorCodeValidate"
                      select="'2513'"/>
      <xsl:with-param name="node"
                      select="$tipoComprobanteReferencia"/>
      <xsl:with-param name="regexp"
                      select="'^(12|03)$'"/>
      <xsl:with-param name="descripcion"
                      select="concat('Error en la linea: ', $nroLinea, '. El tipo de comprobante relacionado debe de ser 12 (ticket) o 03 (boleta)')"/>
    </xsl:call-template>
    <!-- 
      ...
      <xsl:when test="$tipoComprobanteReferencia = '12'">
        ...
        <xsl:call-template name="existAndRegexpValidateElement">
          ...
          <xsl:with-param name="errorCodeValidate"
                          select="'2117'"/>
          <xsl:with-param name="node"
                          select="$comprobanteModificaID"/>
          <xsl:with-param name="regexp"
                          select="'(?!0+-)^[a-zA-Z0-9]{1,20}-(?!0+$)([0-9]{1,20})$'"/>
          <xsl:with-param name="descripcion"
                          select="concat('Error en la linea: ', $nroLinea)"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
        ...
        <xsl:call-template name="existAndRegexpValidateElement">
          ...
          <xsl:with-param name="node"
                          select="$comprobanteModificaID"/>
          <xsl:with-param name="regexp"
                          select="'^([B](?!0+-)[A-Z0-9]{3})-(?!0+$)([0-9]{1,8})$|^(?!0+-)([0-9]{1,4})-(?!0+$)([0-9]{1,8})$'"/>
          <xsl:with-param name="descripcion"
                          select="concat('Error en la linea: ', $nroLinea)"/>
        </xsl:call-template>
      </xsl:otherwise>
