...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:key name="by-invoiceLine-id"
           match="*[local-name()='VoidedDocuments']/sac:VoidedDocumentsLine"
           use="cbc:LineID"/>
  <!-- key comprobantes duplicados por linea -->
  <xsl:key name="by-Billing-in-line"
           match="sac:VoidedDocumentsLine/sac:BillingPayment"
           use="concat(cbc:LineID,cbc:DocumentTypeCode,sac:DocumentSerialID, number(sac:DocumentNumberID))"/>
  ...
  <xsl:template match="/*">
    ...
    <!-- /VoidedDocuments/cbc:IssueDate El valor del Tag UBL es mayor a la fecha de envío
        ERROR 2301 
				** hasta conseguir las librerias para reconocer la funcion seconds() de \error\validate_utils **
    ...
    <xsl:call-template name="isTrueExpresion">
      ...
      <xsl:with-param name="expresion"
                      select="count(ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/sac:AdditionalInformation) &gt; 1"/>
    </xsl:call-template>
  ...
  <xsl:template match="sac:VoidedDocumentsLine">
    ...
    <xsl:call-template name="existAndRegexpValidateElement">
      ...
      <xsl:with-param name="errorCodeValidate"
                      select="'2310'"/>
      <xsl:with-param name="node"
                      select="sac:DocumentSerialID"/>
      <xsl:with-param name="regexp"
                      select="'(^[FS][A-Z0-9]{3}$)|(^[\d]{1,4}$)'"/>
    ...
    <xsl:call-template name="isTrueExpresion">
      ...
      <xsl:with-param name="node"
                      select="cbc:LineID"/>
      <xsl:with-param name="expresion"
                      select="count(key('by-Billing-in-line', concat(cbc:LineID,cbc:DocumentTypeCode,sac:DocumentSerialID, number(sac:DocumentNumberID)))) &gt; 1"/>
      <xsl:with-param name="descripcion"
                      select="concat('Error en la linea:', position(), '. ')"/>
    </xsl:call-template>
    ...
    <xsl:call-template name="existAndRegexpValidateElement">
      ...
      <xsl:with-param name="errorCodeValidate"
                      select="'2314'"/>
      <xsl:with-param name="node"
                      select="sac:VoidReasonDescription"/>
      <xsl:with-param name="regexp"
                      select="'^(?!\s*$)[^\s].{2,}$'"/>
      <xsl:with-param name="descripcion"
                      select="concat('Error en la linea:', $nroLinea, '. ')"/>
