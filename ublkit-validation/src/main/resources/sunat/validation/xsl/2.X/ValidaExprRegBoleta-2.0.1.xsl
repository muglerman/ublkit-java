    ...
    <xsl:template match="/*">
	   ...
	   -->
		...
		<xsl:call-template name="regexpValidateElementIfExist">
			...
			<xsl:with-param name="node" select="cbc:ID"/>
			<!--Pase126 Contingencia js 260718 <xsl:with-param name="regexp" select="'^[B][A-Z0-9]{3}-[0-9]{1,8}?$'"/>-->
			<xsl:with-param name="regexp" select="'^([B][A-Z0-9]{3}|[0-9]{4})-[0-9]{1,8}?$'"/>			
		</xsl:call-template>
        ...
        <!-- Error en validacion de empresas financieras -->
        	...
        	<xsl:call-template name="isTrueExpresion">
	            ...
	            <xsl:with-param name="descripcion" select="concat('cac:AdditionalItemProperty/cbc:NameCode ' , cac:InvoiceLine/cac:Item/cac:AdditionalItemProperty/cbc:NameCode)"/>				
          </xsl:call-template>
        </xsl:if>                                                                 
        ...
        <!--PAS20221U210700304-EBV INI 4438-->
        ...
        <!--PAS20221U210700304-EBV FIN 4438-->

		<!-- PAS20221U210700304 -EBV INICIO Se agrega -->
        ...
        <!-- PAS20221U210700304-EBV INICIO Reemplazamos la tasa fija 0.18 por el parámetro Tasa q identifica la primera del percent dentro de los q son 1000-->
       	<xsl:variable name="totalPrecioVentaCalculadoIGV18" select="$totalValorVenta + $SumatoriaISC + $SumatoriaICBPER + $SumatoriaOtrosTributos + $AnticiposISC + ($MontoBaseIGVLinea - $MontoDescuentoAfectoBI + $MontoCargosAfectoBI) * 0.18"/>
       	<xsl:variable name="totalPrecioVentaCalculadoIGV10" select="$totalValorVenta + $SumatoriaISC + $SumatoriaICBPER + $SumatoriaOtrosTributos + $AnticiposISC + ($MontoBaseIGVLinea - $MontoDescuentoAfectoBI + $MontoCargosAfectoBI) * 0.10"/>
       	
       		<xsl:variable name="indicadorPrecioVentaError18">
		    <xsl:choose>
		        <xsl:when test="($totalPrecioVenta + 1 ) &lt; $totalPrecioVentaCalculadoIGV18 or ($totalPrecioVenta - 1) &gt; $totalPrecioVentaCalculadoIGV18">
		...
		<xsl:variable name="indicadorPrecioVentaError10">
		    <xsl:choose>
		        <xsl:when test="($totalPrecioVenta + 1 ) &lt; $totalPrecioVentaCalculadoIGV10 or ($totalPrecioVenta - 1) &gt; $totalPrecioVentaCalculadoIGV10">
		            <xsl:value-of select="1"/>
		        </xsl:when>
		        <xsl:otherwise>
		            <xsl:value-of select="'0'"/>
		...
		<xsl:variable name="totalPrecioVentaCalculadoSinIgvSinIVAP" select="$totalValorVenta + $SumatoriaISC + $SumatoriaICBPER + $SumatoriaOtrosTributos"/>
       	...
       	<!-- PAS20221U210700304(Traslado) - Excel v7 - Se resta variable de anticipos ISC solo si la operación es afecta al IGV --> 
        ...
        <xsl:variable name="SumatoriaIGVCalculado">
          <xsl:choose>
       	...
       	<!-- PAS20221U210700304-EBV INICIO 4290 - Reincorporación considerando tasas 10% y 18% -->
        <xsl:variable name="sumIGVCalculado18">
          <xsl:choose>
            <xsl:when test="$MontoBaseIGVLinea &gt; 0">
               <xsl:value-of select="($MontoBaseIGVLinea - $MontoDescuentoAfectoBI - $MontoDescuentoAfectoBIAnticipo - $AnticiposISC + $MontoCargosAfectoBI) * 0.18"/>
            </xsl:when>
        ...
        <xsl:variable name="sumIGVCalculado10">
		  <xsl:choose>
		    <xsl:when test="$MontoBaseIGVLinea &gt; 0">
		...
		<xsl:variable name="indicadorError18">
		    <xsl:choose>
		        <xsl:when test="($SumatoriaIGV + 1 ) &lt; $sumIGVCalculado18 or ($SumatoriaIGV - 1) &gt; $sumIGVCalculado18">
		            <xsl:value-of select="1"/>
		        </xsl:when>
		        <xsl:otherwise>
		            <xsl:value-of select="'0'"/>
		...
		<xsl:variable name="indicadorError10">
			<xsl:choose>
				<xsl:when test="($SumatoriaIGV + 1 ) &lt; $sumIGVCalculado10 or ($SumatoriaIGV - 1) &gt; $sumIGVCalculado10">
					<xsl:value-of select="1"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="'0'"/>
        ...
        <!-- PAS20221U210700304-EBV FIN 4290 -->
        ...
        <!--PAS20221U210700304-EBV INICIO 4290 - Habilitamos la validación doble-->
	    ...
	    <xsl:choose>
	      <xsl:when test="$tipoOperacion ='0113'">
	         	<xsl:if test="cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cac:TaxScheme/cbc:ID[text() = '1000']] and cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cac:TaxScheme/cbc:ID[text() = '1000']]/cbc:TaxAmount &gt; 0">
				        <xsl:call-template name="isTrueExpresion">
		...
		<!--PAS20221U210700304-EBV INICIO 4290 - Comentamos para habilitar la misma validación doble en la parte superior-->
       	<!--<xsl:if test="cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cac:TaxScheme/cbc:ID[text() = '1000']]">
			<xsl:call-template name="isTrueExpresion">
	            <xsl:with-param name="errorCodeValidate" select="'4290'" />
	            <xsl:with-param name="node" select="cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cac:TaxScheme/cbc:ID[text() = '1000']]/cbc:TaxAmount" />
        ...
        <!--PAS20221U210700304 INI-EBV- Comentamos OBS 4310 y actualizamos condiciones-->
        ...
        <!--PAS20221U210700304 FIN-EBV- Comentamos OBS 4310 y actualizamos condiciones-->
        
        <!-- PAS20221U210700304-EBV INICIO 4310 - Descomentamos para utilizar la validación doble de 4310-->
        <!--PAS20231U210700073-IMR tipo de operación NRUS 0113 -->
        <xsl:if test="$tipoOperacion != '0113'">
        <xsl:if test="cac:LegalMonetaryTotal/cbc:TaxInclusiveAmount">
           <xsl:choose>
              ...
              <xsl:otherwise>
	                  ...
	                  <xsl:with-param name="expresion" select="not($indicadorPrecioVentaError18 &gt; 0 and $indicadorPrecioVentaError10 = 0) and not($indicadorPrecioVentaError18 = 0 and $indicadorPrecioVentaError10 &gt; 0) and not($indicadorPrecioVentaError18 = 0 and $indicadorPrecioVentaError10 = 0)" />
	                  <xsl:with-param name="isError" select ="false()"/>
	                </xsl:call-template>
              </xsl:otherwise>
           </xsl:choose>
		...
		<!-- PAS20221U210700304-EBV FIN 4310 - Descomentamos para utilizar la validación doble de 4310-->
        ...
        Información Adicional  - Guia Resumen
        
        ===========================================================================================================================================
        -->
                
        <xsl:apply-templates select="cac:Delivery/cac:Shipment/cac:Delivery/cac:DeliveryAddress"/>
        
        <!-- 
        ===========================================================================================================================================
        
        Fin Información Adicional  - Guia Resumen
        ...
        <xsl:call-template name="findElementInCatalogProperty">
			...
			<xsl:with-param name="propiedad" select="'boleta'"/>
        ...
        <!--  <xsl:apply-templates select="cac:Delivery/cac:Shipment/cac:Delivery"/> -->	            
    ...
    *******************************************************************************************************************************************
    ...
    <xsl:template match="cac:Delivery/cac:DeliveryLocation/cac:Address">
        ...
        <xsl:call-template name="regexpValidateElementIfExist">
            ...
            <xsl:with-param name="regexp" select="'^(?!\s*$)[^\s].{1,25}$'"/> <!-- de hasta 25 caracteres que no inicie por espacio -->
            <xsl:with-param name="isError" select ="false()"/>
        </xsl:call-template>
    ...
    <xsl:template match="cac:AccountingCustomerParty">
        ...
        <xsl:if test="cac:Party/cac:PartyIdentification/cbc:ID != '-'">
	        <xsl:choose>
				...
				<xsl:when test="(cac:Party/cac:PartyIdentification/cbc:ID/@schemeID ='4') or (cac:Party/cac:PartyIdentification/cbc:ID/@schemeID ='7') or
...
<!--         </xsl:if> -->
        ...
        <xsl:choose>
            <xsl:when test="$tipoOperacion = '0200' or $tipoOperacion = '0201' or $tipoOperacion = '0203' or $tipoOperacion = '0204' or $tipoOperacion = '0205' or $tipoOperacion = '0205' or $tipoOperacion = '0206' or $tipoOperacion = '0207' or $tipoOperacion = '0208' or $tipoOperacion = '0401'">
            	<xsl:call-template name="isTrueExpresion">
    ...
    <xsl:template match="cac:InvoiceLine">
        ...
        <xsl:call-template name="regexpValidateElementIfExist">
            ...
            <xsl:with-param name="regexp" select="'^[0-9a-zA-Z]{1,30}$'"/> <!-- Texto de un caracter a 250. acepta saltos de linea, no permite que inicie con espacios -->
            <xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea)"/>
            <xsl:with-param name="isError" select ="false()"/>
        </xsl:call-template>
...
<!--         </xsl:if> -->
        ...
        <xsl:call-template name="existAndRegexpValidateElement">
            ...
            <xsl:with-param name="errorCodeValidate" select="'2027'"/>
            <xsl:with-param name="node" select="cac:Item/cbc:Description"/>
            <xsl:with-param name="regexp" select="'^((?!\s*$)[\s\S]{0,499})$'"/> 
            <xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea)"/>
        </xsl:call-template>
        -->
        ...
        <xsl:call-template name="isTrueExpresion">
            ...
            <xsl:with-param name="expresion" select="cac:PricingReference/cac:AlternativeConditionPrice[cbc:PriceTypeCode ='02']/cbc:PriceAmount &gt; 0 and not(cac:TaxTotal[cac:TaxSubtotal[cac:TaxCategory/cac:TaxScheme/cbc:ID[text() = '9996']]/cbc:TaxableAmount &gt; 0])" />
            <xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea)"/>
        </xsl:call-template>
        ...
        <xsl:if test="$tipoOperacion = '1004'">        
	        ...
	        <xsl:apply-templates select="cac:Delivery/cac:DeliveryTerms" mode="linea">
	            <xsl:with-param name="nroLinea" select="$nroLinea"/>
	        </xsl:apply-templates>
    ...
    <xsl:template match="cac:Delivery" mode="linea">
        ...
        <xsl:call-template name="regexpValidateElementIfExist">
            ...
            <xsl:with-param name="regexp" select="'^(?!\s*$)[^\s].{3,200}$'"/> <!-- de tres a 1500 caracteres que no inicie por espacio -->
            <xsl:with-param name="isError" select ="false()"/>
            <xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea)"/>
        </xsl:call-template>
    ...
    ===========================================================================================================================================
    -->        
    <xsl:template match="cac:Delivery/cac:DeliveryTerms" mode="linea">
        <xsl:param name="nroLinea"/>
...
<!-- 	    PAS20191U210000012 -->		
        ...
        <xsl:if test="$codigoTributo = '1000' or $codigoTributo = '1016'">
	    	<xsl:call-template name="isTrueExpresion">
	            <xsl:with-param name="errorCodeValidate" select="'3111'" />
	            <xsl:with-param name="node" select="cbc:TaxAmount" />
	            <xsl:with-param name="expresion" select="cbc:TaxableAmount &gt; 0 and $tipoOperacion != '0113' and cbc:TaxAmount = 0" />
	        	<xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea, ' Tributo: ', $codigoTributo)"/>
	    	</xsl:call-template>
	    </xsl:if>
        ...
        <xsl:if test="$codigoTributo = '1000' or $codigoTributo = '1016'">
	    	<xsl:call-template name="isTrueExpresion">
	            <xsl:with-param name="errorCodeValidate" select="'2993'" />
	            <xsl:with-param name="node" select="cac:TaxCategory/cbc:Percent" />
	            <xsl:with-param name="expresion" select="cbc:TaxableAmount &gt; 0 and $tipoOperacion != '0113' and cac:TaxCategory/cbc:Percent = 0" />
	        	<xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea, ' Tributo: ', $codigoTributo)"/>
	    	</xsl:call-template>
	    </xsl:if>
...
<!--           PAS20191U210000012 add 7152       -->
        ...
        <xsl:if test="$tipoOperacion != '2100' and $tipoOperacion != '2101' and $tipoOperacion != '2102' and $tipoOperacion != '2103' and $tipoOperacion != '2104'">
           <xsl:call-template name="isTrueExpresion">
    ...
    <xsl:template match="cac:Item/cac:AdditionalItemProperty" mode="linea">
      ...
      <!--  
		...
		<xsl:choose>
			...
			<xsl:when test="$codigoConcepto = '7017'">
...
<!--             	<xsl:call-template name="regexpValidateElementIfExist">
	            	<xsl:with-param name="errorCodeValidate" select="'4280'"/>
		            <xsl:with-param name="node" select="cbc:Value"/>
		            <xsl:with-param name="regexp" select="'^(?!\s*$)[^\s].{0,11}$'"/>
					<xsl:with-param name="isError" select ="false()"/>
		            <xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea, ' ConceptoItem ', cbc:NameCode)"/>
		        </xsl:call-template> -->
				
                      <xsl:choose>
                          <xsl:when test="string-length(cbc:Value) &gt; 12 or string-length(cbc:Value) &lt; 1 ">
                                          <xsl:call-template name="isTrueExpresion">
                                      <xsl:with-param name="errorCodeValidate" select="'4280'" />
                                      <xsl:with-param name="node" select="cbc:Value" />
                                      <xsl:with-param name="expresion" select="true()" />
                          ...
                          <xsl:otherwise>
                                  <xsl:call-template name="regexpValidateElementIfExist">
                                     <xsl:with-param name="errorCodeValidate" select="'4280'"/>
                                     <xsl:with-param name="node" select="cbc:Value"/>
                                     <xsl:with-param name="regexp" select="'^[\da-zA-Z]{1,12}$'"/> 
                                     <xsl:with-param name="isError" select ="false()"/>
                                     <xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea, ' ConceptoItem ', cbc:NameCode)"/>
                                  </xsl:call-template>
                          </xsl:otherwise>
			...
			<xsl:when test="$codigoConcepto = '7018'">
            	<xsl:call-template name="existElement">
		            <xsl:with-param name="errorCodeNotExist" select="'3064'"/>
		            <xsl:with-param name="node" select="cbc:Value"/>
		            <xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea, ' ConceptoItem ', cbc:NameCode)"/>
            	...
            	<xsl:call-template name="regexpValidateElementIfExist">
	            	<xsl:with-param name="errorCodeValidate" select="'4280'"/>
		            <xsl:with-param name="node" select="cbc:Value"/>
		            <xsl:with-param name="regexp" select="'^(20)\d\d([- /.])(0[1-9]|1[012])$'"/>
					<xsl:with-param name="isError" select ="false()"/>
		            <xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea, ' ConceptoItem ', cbc:NameCode)"/>
		        </xsl:call-template>
			...
			<xsl:when test="$codigoConcepto = '7019'">
    ...
    <xsl:template match="cac:TaxSubtotal" mode="cabecera">
		...
		<xsl:if test="$codigoTributo = '7152'">
			<xsl:if test="cbc:TaxAmount &gt; 0">
				<xsl:call-template name="isTrueExpresion">
					<xsl:with-param name="errorCodeValidate" select="'3240'" />
					<xsl:with-param name="node" select="cbc:TaxAmount" />
					<xsl:with-param name="expresion" select="$tipoOperacion = '0113'" />
					<xsl:with-param name="descripcion" select="concat('Error Tributo ', $codigoTributo)"/>
				</xsl:call-template>
			</xsl:if>
	    </xsl:if>
    ...
    <xsl:template match="cac:TaxTotal" mode="cabecera">
        ...
        <xsl:variable name="SumatoriaImpuestos" select="sum(cac:TaxSubtotal[cac:TaxCategory/cac:TaxScheme/cbc:ID[text() = '1000' or text() = '7152' or text() = '1016' or text() = '9999' or text() = '2000']]/cbc:TaxAmount)"/>
    ...
    <xsl:template match="cac:AllowanceCharge" mode="cabecera">
		...
		<!--Excel v6 PAS20211U210400011 JGS - Se adiciona Percepciones para que valide sin permitir ceros -->
		<xsl:choose>
		   <xsl:when test="$codigoCargoDescuento = '51' or $codigoCargoDescuento = '52' or $codigoCargoDescuento = '53'">
			  <xsl:call-template name="regexpValidateElementIfExist">
        ...
        <xsl:if test="$codigoCargoDescuento = '51' or $codigoCargoDescuento = '52' or $codigoCargoDescuento = '53'">
        	<xsl:variable name="MontoCalculadoPercepcion" select="cbc:BaseAmount * cbc:MultiplierFactorNumeric * 0.01"/>
        	<xsl:variable name="MontoCalculadoPercepcionRed" select='format-number( round(100*$MontoCalculadoPercepcion) div 100 ,"#####0.00" )' />
    ...
    =========================================== Template cac:Delivery/cac:Shipment/cac:Delivery/cac:DeliveryAddress =========================================== 
    ...
    <xsl:template match="cac:Delivery/cac:Shipment/cac:Delivery/cac:DeliveryAddress">
        ...
        <xsl:call-template name="isTrueExpresion">
            <xsl:with-param name="errorCodeValidate" select="'4238'" />
            <xsl:with-param name="node" select="cbc:CitySubdivisionName" />
            <xsl:with-param name="expresion" select="string-length(cbc:CitySubdivisionName) &gt; 25 or string-length(cbc:CitySubdivisionName) &lt; 1 " />
            <xsl:with-param name="isError" select ="false()"/>
        </xsl:call-template>
        ...
        <xsl:call-template name="regexpValidateElementIfExist">
            <xsl:with-param name="errorCodeValidate" select="'4238'"/>
            <xsl:with-param name="node" select="cbc:CitySubdivisionName"/>
            <xsl:with-param name="regexp" select="'^(?!\s*$)[^\s].{1,25}$'"/> 
            <xsl:with-param name="isError" select ="false()"/>
        </xsl:call-template>
        -->
        <!--  El formato del Tag UBL es diferente a alfanumérico de hasta 30 caracteres ERROR 4096 -->
     <xsl:call-template name="isTrueExpresionIfExist">
            <xsl:with-param name="errorCodeValidate" select="'4239'" />
            <xsl:with-param name="node" select="cbc:CityName" />
            <xsl:with-param name="expresion" select="string-length(cbc:CityName) &gt; 30 or string-length(cbc:CityName) &lt; 1 " />
            <xsl:with-param name="isError" select ="false()"/>
        </xsl:call-template>
     ...
     <xsl:call-template name="isTrueExpresionIfExist">
            <xsl:with-param name="errorCodeValidate" select="'4240'" />
            <xsl:with-param name="node" select="cbc:CountrySubentity" />
            <xsl:with-param name="expresion" select="string-length(cbc:CountrySubentity) &gt; 30 or string-length(cbc:CountrySubentity) &lt; 1 " />
            <xsl:with-param name="isError" select ="false()"/>
        </xsl:call-template>
        ...
        <xsl:call-template name="regexpValidateElementIfExist">
            <xsl:with-param name="errorCodeValidate" select="'4240'"/>
            <xsl:with-param name="node" select="cbc:CountrySubentity"/>
            <xsl:with-param name="regexp" select="'^(?!\s*$)[^\s].{1,29}$'"/> 
            <xsl:with-param name="isError" select ="false()"/>
        </xsl:call-template>
        -->
        ...
        <xsl:call-template name="isTrueExpresionIfExist">
            <xsl:with-param name="errorCodeValidate" select="'4241'" />
            <xsl:with-param name="node" select="cbc:District" />
            <xsl:with-param name="expresion" select="string-length(cbc:District) &gt; 30 or string-length(cbc:District) &lt; 1 " />
            <xsl:with-param name="isError" select ="false()"/>
        </xsl:call-template>
    ...
    <xsl:template match="cac:PaymentTerms">
		...
		<xsl:if test="$tipoOperacion = '1002'">
			<xsl:call-template name="isTrueExpresion">
	            ...
	            <xsl:with-param name="expresion" select="cbc:PaymentMeansID != '004'" />
	        </xsl:call-template>
		</xsl:if>
    	...
    	<xsl:if test="$tipoOperacion = '1003'">
			<xsl:call-template name="isTrueExpresion">
	            <xsl:with-param name="errorCodeValidate" select="'3129'" />
	            <xsl:with-param name="node" select="cbc:PaymentMeansID" />
	            <xsl:with-param name="expresion" select="cbc:PaymentMeansID != '028'" />
	        </xsl:call-template>
		</xsl:if>
		...
		<xsl:if test="$tipoOperacion = '1004'">
			<xsl:call-template name="isTrueExpresion">
	            <xsl:with-param name="errorCodeValidate" select="'3129'" />
	            <xsl:with-param name="node" select="cbc:PaymentMeansID" />
	            <xsl:with-param name="expresion" select="cbc:PaymentMeansID != '027'" />
	        </xsl:call-template>
		</xsl:if>
