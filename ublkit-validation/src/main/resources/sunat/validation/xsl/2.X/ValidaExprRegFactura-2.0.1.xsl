...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:regexp="http://exslt.org/regular-expressions" xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2" xmlns:ds="http://www.w3.org/2000/09/xmldsig#" xmlns:ext="urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2" xmlns:sac="urn:sunat:names:specification:ubl:peru:schema:xsd:SunatAggregateComponents-1" xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2" xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" xmlns:dp="http://www.datapower.com/extensions" xmlns:date="http://exslt.org/dates-and-times" extension-element-prefixes="dp" exclude-result-prefixes="dp" version="1.0">
	...
	<xsl:template match="/*">
		...
		<!-- SUNAT 1.1 comentar dp<xsl:variable name="numeroRuc" select="substring(dp:variable('var://context/cpe/nombreArchivoEnviado'), 1, 11)"/> <xsl:variable name="tipoComprobante" select="substring(dp:variable('var://context/cpe/nombreArchivoEnviado'), 13, 2)"/><xsl:variable name="numeroSerie" select="substring(dp:variable('var://context/cpe/nombreArchivoEnviado'), 16, 4)"/><xsl:variable name="numeroComprobante" select="substring(dp:variable('var://context/cpe/nombreArchivoEnviado'), 21, string-length(dp:variable('var://context/cpe/nombreArchivoEnviado')) - 24)"/>-->
		<!-- INICIO SUNAT -->
		...
		<!-- Variables <xsl:variable name="cbcUBLVersionID" select="cbc:UBLVersionID"/><xsl:variable name="cbcCustomizationID" select="cbc:CustomizationID"/>-->
		...
		<!-- ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/sac:AdditionalInformation/sac:AdditionalMonetaryTotal/cbc:ID El valor del Tag UBL debe tener por lo menos uno de los siguientes valores en el /Invoice: 1001 (Gravada), 1002 (Inafecta), 1003 (Exonerada), 1004 (Gratuita) o 3001 (FISE) ERROR 2047<xsl:call-template name="isTrueExpresion"><xsl:with-param name="errorCodeValidate" select="'2047'" /><xsl:with-param name="node" select="ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/sac:AdditionalInformation/sac:AdditionalMonetaryTotal/cbc:ID" /><xsl:with-param name="expresion" select="not(ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/sac:AdditionalInformation/sac:AdditionalMonetaryTotal/cbc:ID[text()='1001' or text()='1002' or text()='1003' or text()='1004' or text()='3001'])" /></xsl:call-template>-->
		...
		<xsl:call-template name="isTrueExpresion">
			...
			<xsl:with-param name="node" select="cbc:ID"/>
			<xsl:with-param name="expresion" select="count(cbc:Note[@languageLocaleID='1000']) &gt; 1 or             count(cbc:Note[@languageLocaleID='1002']) &gt; 1 or             count(cbc:Note[@languageLocaleID='2000']) &gt; 1 or             count(cbc:Note[@languageLocaleID='2001']) &gt; 1 or             count(cbc:Note[@languageLocaleID='2002']) &gt; 1 or             count(cbc:Note[@languageLocaleID='2003']) &gt; 1 or             count(cbc:Note[@languageLocaleID='2004']) &gt; 1 or             count(cbc:Note[@languageLocaleID='2005']) &gt; 1 or             count(cbc:Note[@languageLocaleID='2006']) &gt; 1 or             count(cbc:Note[@languageLocaleID='2007']) &gt; 1 or             count(cbc:Note[@languageLocaleID='2008']) &gt; 1 or             count(cbc:Note[@languageLocaleID='2009']) &gt; 1 "/>
		</xsl:call-template>
		...
		<!-- <xsl:call-template name="findElementInCatalogProperty"><xsl:with-param name="catalogo" select="'51'"/><xsl:with-param name="propiedad" select="'factura'"/><xsl:with-param name="idCatalogo" select="$tipoOperacion"/><xsl:with-param name="valorPropiedad" select="'1'"/><xsl:with-param name="errorCodeValidate" select="'3206'"/></xsl:call-template>-->
		<xsl:call-template name="regexpValidateElementIfExist">
	...
	<xsl:template match="cac:AccountingSupplierParty">
		...
		<xsl:call-template name="isTrueExpresion">
			...
			<xsl:with-param name="expresion" select="string-length(cac:Party/cac:PartyName/cbc:Name) &gt; 1500"/>
			<!-- de 3 a 1500 caracteres (sin importar que lleve tildes)-->
			<xsl:with-param name="isError" select="false()"/>
		</xsl:call-template>
		<xsl:call-template name="regexpValidateElementIfExist">
			<xsl:with-param name="errorCodeValidate" select="'4092'"/>
		...
		<xsl:call-template name="regexpValidateElementIfExist">
			...
			<!-- de hasta 25 caracteres que no inicie por espacio -->
			<xsl:with-param name="isError" select="false()"/>
		</xsl:call-template>
		<xsl:call-template name="isTrueExpresion">
			<xsl:with-param name="errorCodeValidate" select="'4095'"/>
		...
		<xsl:if test="not(string-length(cac:Party/cac:PartyLegalEntity/cac:RegistrationAddress/cbc:CityName) &gt; 30)">
			<xsl:call-template name="regexpValidateElementIfExist">
				...
				<!-- de 1 a 30 caracteres que no inicie por espacio -->
				<xsl:with-param name="isError" select="false()"/>
		...
		<xsl:call-template name="regexpValidateElementIfExist">
			...
			<xsl:with-param name="regexp" select="'^(?!\s*$)[^\s].{0,}$'"/>
			<!-- de tres a 30 caracteres que no inicie por espacio -->
			<xsl:with-param name="isError" select="false()"/>
		</xsl:call-template>
		<xsl:call-template name="isTrueExpresion">
			<xsl:with-param name="errorCodeValidate" select="'4097'"/>
	...
	<xsl:template match="cac:Delivery/cac:DeliveryLocation/cac:Address">
		<xsl:param name="tipoOperacion" select="'-'"/>
		<!-- tipoOperacion es diferente 0104 Venta interna - Itinerante y existe el tag cac:Delivery/cac:DeliveryLocation/cac:Address OBSERVACION 4263<xsl:if test="not($tipoOperacion ='0104')"><xsl:call-template name="isTrueExpresion"><xsl:with-param name="errorCodeValidate" select="'4263'" /><xsl:with-param name="node" select="cac:AddressLine/cbc:Line" /><xsl:with-param name="expresion" select="cac:AddressLine/cbc:Line" /><xsl:with-param name="isError" select ="false()"/></xsl:call-template></xsl:if>-->
		<xsl:choose>
		...
		<!-- <xsl:call-template name="regexpValidateElementIfExist"><xsl:with-param name="errorCodeValidate" select="'4240'"/><xsl:with-param name="node" select="cbc:CountrySubentity"/><xsl:with-param name="regexp" select="'^(?!\s*$)[^\s].{0,29}$'"/> <xsl:with-param name="isError" select ="false()"/></xsl:call-template>-->
		<xsl:choose>
		...
		<!--<xsl:call-template name="regexpValidateElementIfExist"><xsl:with-param name="errorCodeValidate" select="'4241'"/><xsl:with-param name="node" select="cbc:District"/><xsl:with-param name="regexp" select="'^(?!\s*$)[^\s].{0,29}$'"/> <xsl:with-param name="isError" select ="false()"/></xsl:call-template>-->
		<xsl:choose>
	...
	<xsl:template match="cac:AccountingCustomerParty">
		...
		<!-- El Tag UBL es diferente al listado ERROR 2016  TODO agregar la validacion contra el catalogo-->
		<!-- <xsl:if test="cac:Party/cac:PartyIdentification/cbc:ID/@schemeID != '-'"><xsl:call-template name="findElementInCatalog"><xsl:with-param name="errorCodeValidate" select="'2016'"/><xsl:with-param name="idCatalogo" select="cac:Party/cac:PartyIdentification/cbc:ID/@schemeID"/><xsl:with-param name="catalogo" select="'06'"/></xsl:call-template>-->
		<!-- </xsl:if> -->
		<xsl:choose>
			<xsl:when test="$tipoOperacion = '0200' or $tipoOperacion = '0201' or $tipoOperacion = '0203' or $tipoOperacion = '0204' or $tipoOperacion = '0205' or $tipoOperacion = '0206' or $tipoOperacion = '0207' or $tipoOperacion = '0208'">
				<xsl:call-template name="isTrueExpresion">
	...
	<xsl:template match="cac:InvoiceLine">
		...
		<xsl:call-template name="regexpValidateElementIfExist">
			...
			<xsl:with-param name="regexp" select="'^((?!\s*$)[\s\S]{0,29})$'"/>
		...
		<xsl:if test="cac:Item/cac:AdditionalItemProperty/cbc:NameCode[text() = '3050']">
			<xsl:choose>
				<xsl:when test="string-length(cac:Item/cac:AdditionalItemProperty/cbc:Value) &gt; 20 or string-length(cac:Item/cac:AdditionalItemProperty/cbc:Value) &lt; 1 ">
					<xsl:call-template name="isTrueExpresion">
						<xsl:with-param name="errorCodeValidate" select="'4280'"/>
						<xsl:with-param name="node" select="cac:Item/cac:AdditionalItemProperty/cbc:Value"/>
						<xsl:with-param name="expresion" select="true()"/>
		...
		<xsl:if test="cac:Item/cac:AdditionalItemProperty/cbc:NameCode[text() = '3051']">
			<xsl:choose>
				<xsl:when test="string-length(cac:Item/cac:AdditionalItemProperty/cbc:Value) &gt; 20 or string-length(cac:Item/cac:AdditionalItemProperty/cbc:Value) &lt; 3 ">
					<xsl:call-template name="regexpValidateElementIfExist">
						<xsl:with-param name="errorCodeValidate" select="'4280'"/>
						<xsl:with-param name="node" select="cac:Item/cac:AdditionalItemProperty/cbc:Value"/>
				...
				<xsl:otherwise>
					<xsl:call-template name="regexpValidateElementIfExist">
						<xsl:with-param name="errorCodeValidate" select="'4280'"/>
						<xsl:with-param name="node" select="cac:Item/cac:AdditionalItemProperty/cbc:Value"/>
						<xsl:with-param name="regexp" select="'^(?!\s*$)[^\s].{3,}$'"/>
						<xsl:with-param name="isError" select="false()"/>
						<xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea, ' Concepto 3051 ', cbc:Value)"/>
					</xsl:call-template>
				</xsl:otherwise>
		...
		<xsl:if test="cac:Item/cac:AdditionalItemProperty/cbc:NameCode[text() = '3052']">
			<xsl:choose>
				<xsl:when test="string-length(cac:Item/cac:AdditionalItemProperty/cbc:Value) &gt; 15 or string-length(cac:Item/cac:AdditionalItemProperty/cbc:Value) &lt; 3 ">
					<xsl:call-template name="regexpValidateElementIfExist">
						<xsl:with-param name="errorCodeValidate" select="'4280'"/>
						<xsl:with-param name="node" select="cac:Item/cac:AdditionalItemProperty/cbc:Value"/>
						<xsl:with-param name="regexp" select="true()"/>
		...
		<xsl:if test="cac:Item/cac:AdditionalItemProperty/cbc:NameCode[text() = '3054']">
			<xsl:choose>
				<xsl:when test="string-length(cac:Item/cac:AdditionalItemProperty/cbc:Value) &gt; 200 or string-length(cac:Item/cac:AdditionalItemProperty/cbc:Value) &lt; 3 ">
					<xsl:call-template name="regexpValidateElementIfExist">
						<xsl:with-param name="errorCodeValidate" select="'4280'"/>
						<xsl:with-param name="node" select="cac:Item/cac:AdditionalItemProperty/cbc:Value"/>
						<xsl:with-param name="regexp" select="true()"/>
		...
		<xsl:if test="cac:Item/cac:AdditionalItemProperty/cbc:NameCode[text() = '3055']">
			<xsl:call-template name="findElementInCatalog">
				<xsl:with-param name="catalogo" select="'13'"/>
				<xsl:with-param name="idCatalogo" select="cac:Item/cac:AdditionalItemProperty/cbc:Value"/>
				<xsl:with-param name="errorCodeValidate" select="'4280'"/>
				<xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea, ' Concepto 3055 ', cbc:Value)"/>
				<xsl:with-param name="isError" select="false()"/>
			</xsl:call-template>
		</xsl:if>
		<xsl:if test="cac:Item/cac:AdditionalItemProperty/cbc:NameCode[text() = '3056']">
			<xsl:choose>
				<xsl:when test="string-length(cac:Item/cac:AdditionalItemProperty/cbc:Value) &gt; 200 or string-length(cac:Item/cac:AdditionalItemProperty/cbc:Value) &lt; 3 ">
					<xsl:call-template name="regexpValidateElementIfExist">
						<xsl:with-param name="errorCodeValidate" select="'4280'"/>
		...
		<xsl:if test="cac:Item/cac:AdditionalItemProperty/cbc:NameCode[text() = '3057']">
			<xsl:call-template name="findElementInCatalog">
				<xsl:with-param name="catalogo" select="'13'"/>
				<xsl:with-param name="idCatalogo" select="cac:Item/cac:AdditionalItemProperty/cbc:Value"/>
				<xsl:with-param name="errorCodeValidate" select="'4280'"/>
				<xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea, ' Concepto 3057 ', cbc:Value)"/>
				<xsl:with-param name="isError" select="false()"/>
			</xsl:call-template>
		</xsl:if>
		<xsl:if test="cac:Item/cac:AdditionalItemProperty/cbc:NameCode[text() = '3058']">
			<xsl:choose>
				<xsl:when test="string-length(cac:Item/cac:AdditionalItemProperty/cbc:Value) &gt; 200 or string-length(cac:Item/cac:AdditionalItemProperty/cbc:Value) &lt; 3 ">
					<xsl:call-template name="regexpValidateElementIfExist">
						<xsl:with-param name="errorCodeValidate" select="'4280'"/>
		...
		<!--xsl:variable name="cacInvoicePeriodcbcStartDate" select="date:seconds($fechaIngreso)" /><xsl:variable name="cacInvoicePeriodcbcEndDate" select="date:seconds($fechaSalida)" / -->
	...
	<xsl:template match="cac:TaxSubtotal" mode="linea">
		...
		<!-- <xsl:choose>-->
		<!--<xsl:when test="$codigoTributo = '2000' or $codigoTributo = '9999'"><xsl:call-template name="existElement"><xsl:with-param name="errorCodeNotExist" select="'2038'"/><xsl:with-param name="node" select="cac:TaxCategory/cac:TaxScheme/cbc:Name"/><xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea, ' Tributo: ', $codigoTributo)"/></xsl:call-template></xsl:when> -->
		<!--<xsl:otherwise>-->
		<!-- PAS20191U210000012 -->
		<xsl:call-template name="existElement">
	...
	<xsl:template match="cac:TaxTotal" mode="linea">
		...
		<xsl:if test="$BaseIGVIVAPxLinea">
			<xsl:call-template name="isTrueExpresion">
				...
				<xsl:with-param name="node" select="cac:TaxSubtotal[cac:TaxCategory/cac:TaxScheme/cbc:ID[text() = '1000' or text() = '1016']]/cbc:TaxableAmount"/>
	...
	<xsl:template match="cac:AllowanceCharge" mode="linea">
		...
		<!-- Codigo de tributo por linea, el valor del Tag UBL es diferente al listado ERROR 2036 -->
		<xsl:call-template name="existElement">
	...
	<xsl:template match="cac:Item/cac:AdditionalItemProperty" mode="linea">
		...
		<xsl:choose>
			...
			<xsl:when test="$codigoConcepto = '3001'">
				...
				<!-- <xsl:call-template name="regexpValidateElementIfExist"><xsl:with-param name="errorCodeValidate" select="'4280'"/><xsl:with-param name="node" select="cbc:Value"/><xsl:with-param name="regexp" select="'^(?!\s*$)[^\s].{0,14}$'"/> <xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea, ' ConceptoItem ', cbc:NameCode)"/><xsl:with-param name="isError" select ="false()"/></xsl:call-template>-->
			</xsl:when>
			<xsl:when test="$codigoConcepto = '3002'">
				...
				<!-- <xsl:call-template name="regexpValidateElementIfExist"><xsl:with-param name="errorCodeValidate" select="'4280'"/><xsl:with-param name="node" select="cbc:Value"/><xsl:with-param name="regexp" select="'^(?!\s*$)[^\s].{0,99}$'"/> <xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea, ' ConceptoItem ', cbc:NameCode)"/><xsl:with-param name="isError" select ="false()"/></xsl:call-template>-->
			</xsl:when>
			<xsl:when test="$codigoConcepto = '3003'">
				...
				<!--<xsl:call-template name="regexpValidateElementIfExist"><xsl:with-param name="errorCodeValidate" select="'4280'"/><xsl:with-param name="node" select="cbc:Value"/><xsl:with-param name="regexp" select="'^(?!\s*$)[^\s].{0,149}$'"/><xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea, ' ConceptoItem ', cbc:NameCode)"/><xsl:with-param name="isError" select ="false()"/></xsl:call-template> -->
			</xsl:when>
			...
			<xsl:when test="$codigoConcepto = '4060'">
				...
				<!-- <xsl:call-template name="regexpValidateElementIfExist"><xsl:with-param name="errorCodeValidate" select="'4280'"/><xsl:with-param name="node" select="cbc:Value"/><xsl:with-param name="regexp" select="'^(?!\s*$)[^\s].{2,29}$'"/> <xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea, ' ConceptoItem ', cbc:NameCode)"/><xsl:with-param name="isError" select ="false()"/></xsl:call-template>-->
			</xsl:when>
			<xsl:when test="$codigoConcepto = '4061'">
				...
				<!-- <xsl:call-template name="regexpValidateElementIfExist"><xsl:with-param name="errorCodeValidate" select="'4280'"/><xsl:with-param name="node" select="cbc:Value"/><xsl:with-param name="regexp" select="'^(?!\s*$)[^\s].{2,9}$'"/> <xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea, ' ConceptoItem ', cbc:NameCode)"/><xsl:with-param name="isError" select ="false()"/></xsl:call-template>-->
			</xsl:when>
			...
			<xsl:when test="$codigoConcepto = '5001'">
				...
				<!-- <xsl:call-template name="regexpValidateElementIfExist"><xsl:with-param name="errorCodeValidate" select="'4280'"/><xsl:with-param name="node" select="cbc:Value"/><xsl:with-param name="regexp" select="'^(?!\s*$)[^\s].{0,9}$'"/> <xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea, ' ConceptoItem ', cbc:NameCode)"/><xsl:with-param name="isError" select ="false()"/></xsl:call-template>-->
			</xsl:when>
			<xsl:when test="$codigoConcepto = '5002'">
				...
				<!-- <xsl:call-template name="regexpValidateElementIfExist"><xsl:with-param name="errorCodeValidate" select="'4280'"/><xsl:with-param name="node" select="cbc:Value"/><xsl:with-param name="regexp" select="'^(?!\s*$)[^\s].{0,29}$'"/> <xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea)"/><xsl:with-param name="isError" select ="false()"/></xsl:call-template>-->
			</xsl:when>
			...
			<xsl:when test="$codigoConcepto = '7003'">
				...
				<!-- <xsl:call-template name="regexpValidateElementIfExist"><xsl:with-param name="errorCodeValidate" select="'4280'"/><xsl:with-param name="node" select="cbc:Value"/><xsl:with-param name="regexp" select="'^(?!\s*$)[^\s].{2,49}$'"/> <xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea, ' ConceptoItem ', cbc:NameCode)"/><xsl:with-param name="isError" select ="false()"/></xsl:call-template>-->
			</xsl:when>
	...
	<xsl:template match="cac:TaxSubtotal" mode="cabecera">
		...
		<!-- Codigo de tributo por linea, el valor del Tag UBL es diferente al listado ERROR 2054 -->
		<xsl:call-template name="existElement">
			<xsl:with-param name="errorCodeNotExist" select="'2054'"/>
			<xsl:with-param name="node" select="cac:TaxCategory/cac:TaxScheme/cbc:Name"/>
			<xsl:with-param name="descripcion" select="concat('Error Tributo ', $codigoTributo)"/>
		</xsl:call-template>
		<!--  El valor del Tag UBL es diferente al listado segun el codigo de tributo ERROR 2964 -->
		...
		<!-- Codigo de tributo por linea, el valor del Tag UBL es diferente al listado ERROR 2052 -->
		<xsl:call-template name="existElement">
		...
		<!--  El valor del Tag UBL es diferente al listado segun el codigo de tributo ERROR 2961 -->
		<!-- <xsl:call-template name="findElementInCatalogProperty"><xsl:with-param name="catalogo" select="'05'"/><xsl:with-param name="propiedad" select="'UN_ECE_5153'"/><xsl:with-param name="idCatalogo" select="cac:TaxCategory/cac:TaxScheme/cbc:ID"/><xsl:with-param name="valorPropiedad" select="cac:TaxCategory/cac:TaxScheme/cbc:TaxTypeCode"/><xsl:with-param name="errorCodeValidate" select="'2961'"/><xsl:with-param name="descripcion" select="concat('Error Tributo ', $codigoTributo)"/></xsl:call-template>-->
	...
	<xsl:template match="cac:TaxTotal" mode="cabecera">
		...
		<!-- <xsl:if test="$root/cac:InvoiceLine/cac:PricingReference/cac:AlternativeConditionPrice/cbc:PriceTypeCode ='02'"><xsl:call-template name="isTrueExpresion"><xsl:with-param name="errorCodeValidate" select="'2641'" /><xsl:with-param name="node" select="cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme[cbc:ID = '9996']/cbc:ID" /><xsl:with-param name="expresion" select="not(cac:TaxSubtotal[cac:TaxCategory/cac:TaxScheme/cbc:ID[text() = '9996']]) or cac:TaxSubtotal[cac:TaxCategory/cac:TaxScheme/cbc:ID[text() = '9996']]/cbc:TaxableAmount = 0" /></xsl:call-template></xsl:if>-->
		<xsl:call-template name="isTrueExpresion">
	...
	<xsl:template match="cac:AllowanceCharge" mode="cabecera">		
		...
		<!-- <xsl:when test="$codigoCargoDescuento = '00' or $codigoCargoDescuento = '01' or $codigoCargoDescuento = '02' or $codigoCargoDescuento = '03'"> -->
		<xsl:if test="$codigoCargoDescuento = '02' or $codigoCargoDescuento = '03' or $codigoCargoDescuento = '04' or $codigoCargoDescuento = '05' or $codigoCargoDescuento = '06'">
			<xsl:call-template name="isTrueExpresion">
				<xsl:with-param name="errorCodeValidate" select="'3114'"/>
				<xsl:with-param name="node" select="cbc:ChargeIndicator"/>
				<xsl:with-param name="expresion" select="cbc:ChargeIndicator/text() = 'true'"/>
		...
		<xsl:if test="$codigoCargoDescuento != '45' and $codigoCargoDescuento != '51' and $codigoCargoDescuento != '52' and $codigoCargoDescuento != '53' and $codigoCargoDescuento != '62'">
			...
			<xsl:call-template name="isTrueExpresion">
				...
				<xsl:with-param name="errorCodeValidate" select="'4322'"/>
				<xsl:with-param name="node" select="cbc:Amount"/>
	...
	<xsl:template match="cac:Delivery/cac:Shipment">
		...
		<!--<xsl:call-template name="existElement"><xsl:with-param name="errorCodeNotExist" select="'4125'"/><xsl:with-param name="node" select="cac:ShipmentStage/cbc:TransportModeCode"/><xsl:with-param name="isError" select ="false()"/></xsl:call-template>-->
		<xsl:call-template name="findElementInCatalog">
		...
		<!--<xsl:if test="$tipoOperacion = '0111'"><xsl:call-template name="isTrueExpresion"><xsl:with-param name="errorCodeValidate" select="'4134'" /><xsl:with-param name="node" select="cac:ShipmentStage/cbc:TransportModeCode" /><xsl:with-param name="expresion" select="cac:ShipmentStage/cbc:TransportModeCode" /><xsl:with-param name="isError" select ="false()"/></xsl:call-template></xsl:if>-->
		<xsl:call-template name="regexpValidateElementIfExist">
			<xsl:with-param name="errorCodeValidate" select="'4252'"/>
		...
		<xsl:if test="cac:ShipmentStage/cac:CarrierParty/cac:PartyIdentification/cbc:ID">
			<!-- <xsl:call-template name="existAndRegexpValidateElement"><xsl:with-param name="errorCodeNotExist" select="'4161'"/><xsl:with-param name="errorCodeValidate" select="'4162'"/><xsl:with-param name="node" select="cac:ShipmentStage/cac:CarrierParty/cac:PartyIdentification/cbc:ID/@schemeID"/><xsl:with-param name="regexp" select="'^(6)$'"/><xsl:with-param name="isError" select ="false()"/></xsl:call-template>-->
			<xsl:call-template name="existElementNoVacio">
		...
		<!--<xsl:call-template name="existAndRegexpValidateElement"><xsl:with-param name="errorCodeNotExist" select="'4164'"/><xsl:with-param name="errorCodeValidate" select="'4165'"/><xsl:with-param name="node" select="cac:ShipmentStage/cac:CarrierParty/cac:PartyLegalEntity/cbc:RegistrationName"/><xsl:with-param name="regexp" select="'^(?!\s*$)[^\s].{3,199}$'"/><xsl:with-param name="isError" select ="false()"/></xsl:call-template>-->
		...
		<!--<xsl:call-template name="existAndRegexpValidateElement"><xsl:with-param name="errorCodeNotExist" select="'4161'"/><xsl:with-param name="errorCodeValidate" select="'4162'"/><xsl:with-param name="node" select="cac:ShipmentStage/cac:DriverPerson/cbc:ID/@schemeID"/><xsl:with-param name="regexp" select="'^(1|4|7|A)$'"/><xsl:with-param name="isError" select ="false()"/></xsl:call-template>-->
		<xsl:call-template name="regexpValidateElementIfExist">
			<xsl:with-param name="errorCodeValidate" select="'4255'"/>
	...
	<xsl:template match="cac:PrepaidPayment" mode="cabecera">
		<xsl:param name="root"/>
		<!-- /Invoice/cac:PrepaidPayment/cbc:ID Si "Monto anticipado" existe y no existe el Tag UBL OBSERV 3211 -->
		<xsl:call-template name="isTrueExpresion">
	...
	<xsl:template match="cac:PaymentMeans">
		...
		<xsl:if test="$tipoOPeracion = '0302'">
			<!-- <xsl:call-template name="existElement"><xsl:with-param name="errorCodeNotExist" select="'3173'"/><xsl:with-param name="node" select="cbc:PaymentMeansCode"/></xsl:call-template>-->
			<xsl:call-template name="existElement">
